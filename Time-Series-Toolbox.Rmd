...
```{r}
 ...
 plot(BoxCox(mcopper,lambda_mcopper))
 
 plot(enplanements)
 lambda_enplanements = BoxCox.lambda(enplanements)
 plot(BoxCox(enplanements,lambda_enplanements))
```
...
```{r}
 ...
 plot(BoxCox(cangas,lambda_cangas))
```
Your answer:
...
Your answer: Residuals sind nicht nur white noise (Correlogram) -> kein gutes Modell f?r eine Vorhersage
...
Your answer: False, abh?ngig vom Mittleren Abweichungsquadrat

> d. If your model doesn't forecast well, you should make it more complicated.

Your answer: False, eventuell ist das Modell nur durch Ausrei?er oder schlechte Vorverarbeitung schlecht.

> e. Always choose the model with the best forecast accuracy as measured on the test set.

Your answer: Nicht eindeutig l?sbar, da abh?ngig von der ?hnlichkeit zwischen Test- und Realdaten
...
```
Wei?t ?hnlichkeiten zu white noise auf, jedoch nicht eindeutig identifizierbar
...
```
Kein eindeutiges Indiz f?r white noise, Korrelation ?ndert sich von positiv zu negativ
